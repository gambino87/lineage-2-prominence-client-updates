...
class LauncherUpdateTests {
 ...
 static int Main(string[] args){
  ...
  var payload=new LauncherPayload {Version="99.0.0",Channel=LauncherUpdate.Channel,Size=new FileInfo(package).Length,Sha256=Patcher.FileHash(package),ExeSha256=Patcher.Hash(exe)};
  ...
  using(var rsa=new RSACryptoServiceProvider(2048)){rsa.PersistKeyInCsp=false;var settings=new Settings{Feed=new Uri(manifest).AbsoluteUri,PublicKey=rsa.ToXmlString(false),AllowLocalFeed=true};
   ...
   payload.Version=LauncherUpdate.Version;sign();Check(!LauncherUpdate.IsRequired(payload));Check(LauncherUpdate.Prepare(settings,home)==null);
   payload.Version="1.0.0";sign();Check(!LauncherUpdate.IsRequired(payload));Check(LauncherUpdate.Prepare(settings,home)==null);
   payload.Version="invalid";Reject(()=>LauncherUpdate.IsRequired(payload));
   payload.Version="99.0.0";sign();Check(LauncherUpdate.IsRequired(payload));string staged=LauncherUpdate.Prepare(settings,home);Check(Patcher.FileHash(staged)==payload.ExeSha256);Check(File.ReadAllText(target)=="old launcher");
