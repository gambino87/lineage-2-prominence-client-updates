...
class Integration {
 ...
 static void Main(string[]args) {
  ...
  using(var rsa=new RSACryptoServiceProvider(2048)){rsa.PersistKeyInCsp=false;Patcher.WriteJson(mf,manifest);Sign(mf,rsa);
   ...
   Check(p.FilesHashed==0 && p.FilesReused==3,"Play reuses verified unchanged files");
   var reopened=new Patcher(settings){TestMode=true};reopened.LoadRelease();Check(reopened.Check().Count==0 && reopened.FilesHashed==0 && reopened.FilesReused==3,"Verification cache survives restarting the launcher");
   reopened.Check(true);Check(reopened.FilesHashed==3 && reopened.FilesReused==0,"Full verification bypasses every cached file");
   string cachePath=Path.Combine(settings.ClientDirectory,".launcher/verification.json");File.WriteAllText(cachePath,"invalid cache");reopened.Check();Check(reopened.FilesHashed==3,"Invalid verification cache falls back to hashing");
   string originalHost=settings.ServerAddress;settings.ServerAddress="100.80.20.11";var hostChanges=reopened.Check();Check(hostChanges.Count==1 && hostChanges[0].Path=="system/psetup.dll" && reopened.FilesHashed==1,"Connection address changes invalidate the DLL cache only");settings.ServerAddress=originalHost;reopened.Check();
   ...
   DateTime originalTime=File.GetLastWriteTimeUtc(live);File.WriteAllBytes(live,new byte[v1.Length]);File.SetLastWriteTimeUtc(live,originalTime.AddSeconds(5));Check(p.Check().Count==1 && p.FilesHashed==1,"Same-size file edits are detected from changed metadata");p.Apply();
   originalTime=File.GetLastWriteTimeUtc(live);File.WriteAllBytes(live,new byte[v1.Length]);File.SetLastWriteTimeUtc(live,originalTime);p.Apply(true);Check(File.ReadAllBytes(live).SequenceEqual(v1),"Repair catches corruption even when size and timestamps are preserved");
   ...
   manifest.Version="test2";Patcher.WriteJson(mf,manifest);Sign(mf,rsa);p.LoadRelease();Check(p.Check().Count==1 && p.FilesHashed==1,"New signed file hashes invalidate only changed client files");p.FailAfter=1;Fails(()=>p.Apply(),"Failure injection");Check(File.ReadAllBytes(live).SequenceEqual(v1),"Rollback restores previous file");p.FailAfter=-1;p.Apply();Check(File.ReadAllBytes(live).SequenceEqual(v2),"Retry after rollback");
   manifest.Version="test2-launcher-only";Patcher.WriteJson(mf,manifest);Sign(mf,rsa);p.LoadRelease();Check(p.Check().Count==0 && p.FilesHashed==0,"Launcher-only release versions do not trigger a new client hash scan");
