...
an in-launcher download or installation. The existing Install flow is unchanged. Launcher 1.1.5 removes the downloaded-ZIP picker from both environments.
...
2. Wait for the folder check. The primary button says **Install** for a new installation or **Update** when managed files need updating. It says **Up to date** and is disabled when no changes are needed; it is also disabled while checking.
