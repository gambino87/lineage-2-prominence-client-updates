...
class Window : Form {
 ...
 readonly Button update=new Button(),repair=new Button(),play=new Button(),browse=new Button(),archive=new Button(),check=new Button();
 bool busy;
 public Window(Settings config,string path,bool preview=false) {
  settings=config;settingsPath=path;patcher=new Patcher(settings);
  ...
  MakeButton(update,"Install / Update",30,478,200);update.Click+=async delegate {await Run(true,false);};
  ...
  folder.TextChanged+=delegate{play.Enabled=false;};host.TextChanged+=delegate{play.Enabled=false;};
  ...
  if(!preview)Shown+=async delegate {if(!string.IsNullOrWhiteSpace(settings.Feed))await Run(false,false);};
 }
 ...
 async Task Run(bool apply,bool launch) {
  if(busy)return;busy=true;foreach(var b in new[]{update,repair,play,browse,archive,check})b.Enabled=false;folder.Enabled=host.Enabled=false;
  try {
   ...
   bool ready=await Task.Run(()=>{patcher.LoadRelease();if(apply)patcher.Apply();if(launch){patcher.Play();return true;}return patcher.Check().Count==0;});
   notes.Text=patcher.Release.Notes;version.Text="Client release "+patcher.Release.Version;play.Enabled=ready;
  ...
  finally {busy=false;foreach(var b in new[]{update,repair,browse,archive,check})b.Enabled=true;folder.Enabled=host.Enabled=true;}
 }
